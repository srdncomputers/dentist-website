<script>
  ...
  // ── CONFIG ──
  const CLINIC_NAME    = "Kamakhya Dental And Implant Centre";
  const DOCTOR_1_NAME  = "Dr. Deepmala Rathore";
  const DOCTOR_1_QUAL  = "BDS — Senior Dental Consultant";
  const DOCTOR_2_NAME  = "Dr. Advait Singh Rathore";
  const DOCTOR_2_QUAL  = "BDS";
  const WHATSAPP_NUM   = "917217797563";
  const PHONE_1        = "7217797563";
  const PHONE_2        = "9211204233";
  const CITY           = "New Delhi";
  const ADDRESS        = "Shop No. 2 & 3, Sector 12, R.K. Puram, New Delhi – 110022";
  const EMAIL          = "info@kamakhyadental.in";
  const CLOUDINARY_CLOUD  = "dmldsl2q4";
  const CLOUDINARY_PRESET = "dental_gallery";
  const ADMIN_PASSWORD    = "kamakhya2024";
  ...
  function saveGallery() { localStorage.setItem('dental_gallery', JSON.stringify(galleryImages)); }

  function unlockAdmin() {
    if (adminPassword === ADMIN_PASSWORD) { adminUnlocked = true; adminError = ''; }
    else adminError = 'Incorrect password. Please try again.';
  }
  ...
  async function handleUpload(e) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (data.secure_url) {
        galleryImages = [...galleryImages, { id: data.public_id, url: data.secure_url, label: '', uploadedAt: new Date().toLocaleDateString('en-IN') }];
        saveGallery();
      }
  ...
  function waLink(msg = '') {
    return `https://wa.me/${WHATSAPP_NUM}?text=${encodeURIComponent(msg || `Hi, I'd like to book an appointment at ${CLINIC_NAME}.`)}`;
  }
  ...
  const testimonials = [
    ...
    { name: 'Rahul Gupta', stars: 5, text: 'Had an implant done here — completely painless! The doctors explained every step. Highly recommend.' },
...
<svelte:head>
  <title>{CLINIC_NAME} | R.K. Puram, New Delhi</title>
  <meta name="description" content="{DOCTOR_1_NAME} & {DOCTOR_2_NAME} – {CLINIC_NAME} in R.K. Puram, New Delhi. Expert dental care, implants, teeth whitening and more. Book via WhatsApp.">
...
<section id="home" class="hero">
  ...
  <div class="hero-content">
    <div class="hero-badge">✦ Trusted Dental Care in R.K. Puram, New Delhi</div>
    <h1>Your Perfect <em>Smile</em><br>Starts Here</h1>
    <p class="hero-sub">Expert dental treatments and implants delivered with precision, comfort and care. From routine cleanings to complete smile makeovers.</p>
    ...
    <div class="hero-stats">
      <div class="stat"><span class="stat-num">5000+</span><span class="stat-label">Happy Patients</span></div>
  ...
  <div class="hero-visual">
    <div class="hero-card">
      <!-- Doctor 1 -->
      <div class="hcard-top">
        <div class="hcard-avatar">👩‍⚕️</div>
        <div>
          <div class="hcard-name">{DOCTOR_1_NAME}</div>
          <div class="hcard-qual">{DOCTOR_1_QUAL}</div>
        </div>
      </div>
      <!-- Doctor 2 -->
      <div class="hcard-top" style="margin-top:12px">
        <div class="hcard-avatar">👨‍⚕️</div>
        <div>
          <div class="hcard-name">{DOCTOR_2_NAME}</div>
          <div class="hcard-qual">{DOCTOR_2_QUAL}</div>
        </div>
      </div>
      <div class="hcard-divider"></div>
      <div class="hcard-row"><span class="hcard-dot green"></span> Clinic Open Today</div>
      <div class="hcard-row"><span class="hcard-dot blue"></span> Mon, Wed–Sun · 10:00 AM – 8:00 PM</div>
      <div class="hcard-row"><span class="hcard-dot red"></span> Tuesday — Closed</div>
      <a href={waLink()} target="_blank" class="hcard-btn">Chat on WhatsApp →</a>
    </div>
  </div>
</section>
...
<section id="services" class="section services-section">
  <div class="section-header">
    ...
    <p>From preventive care to advanced implant procedures — we offer everything your smile needs under one roof.</p>
  </div>
  <div class="services-grid">
    {#each services as s}
      <div class="service-card">
        ...
        <h3>{s.title}</h3>
        <p>{s.desc}</p>
        <a href={waLink(`Hi, I'd like to know more about ${s.title} at Kamakhya Dental.`)} target="_blank" class="svc-link">Book This →</a>
      </div>
    {/each}
  </div>
</section>
...
<section id="gallery" class="section gallery-section">
    ...
    <div class="gallery-grid">
      {#each galleryImages as img}
        <div class="gallery-item">
          ...
          {#if img.label}<div class="gallery-label">{img.label}</div>{/if}
        </div>
      {/each}
    </div>
  {:else}
  ...
  <div class="gallery-cta">
    <a href={waLink('Hi, I\'d like to know more about smile makeovers at Kamakhya Dental.')} target="_blank" class="btn-primary">Want a Transformation? Chat With Us →</a>
  </div>
</section>
...
<section id="about" class="section about-section">
  <div class="about-visual">
    <div class="about-card">
      ...
      <div class="about-badge">✦ Kamakhya Dental And Implant Centre</div>
      <div class="about-highlights">
        <div class="ah-item"><span class="ah-num">10+</span><span class="ah-label">Years Practice</span></div>
        <div class="ah-item"><span class="ah-num">5000+</span><span class="ah-label">Smiles Made</span></div>
  ...
  <div class="about-content">
    ...
    <h2>Expert Care by <em>Skilled Hands</em></h2>
    <p><strong>{DOCTOR_1_NAME} (BDS)</strong> — Senior Dental Consultant with extensive experience in general and cosmetic dentistry. She is known for her patient-first approach and gentle, painless treatments.</p>
    <p><strong>{DOCTOR_2_NAME} (BDS)</strong> — Specialist in dental implants and restorative procedures, bringing precision and modern techniques to every treatment.</p>
    ...
    <div class="about-features">
      {#each ['Dental Implants by specialist doctors', 'Painless treatments with latest technology', 'Transparent pricing — no hidden costs', 'Child-friendly & family dental care', 'Emergency appointments available', 'Sterilised, hygienic environment'] as f}
...
<section id="contact" class="section contact-section">
  ...
  <div class="contact-grid">
    <div class="contact-card wa-card">
      ...
      <a href={waLink()} target="_blank" class="btn-primary wa-btn">Open WhatsApp Chat →</a>
    </div>
    <div class="contact-info">
      ...
      <div class="ci-item">
        ...
        <div>
          ...
          Mon, Wed – Sun: 10:00 AM – 8:00 PM<br>
          <span style="color:#ef4444;font-weight:600">Tuesday: Closed</span>
        </div>
      </div>
      <div class="ci-item">
        <span class="ci-icon">📞</span>
    ...
    <div class="admin-box">
      ...
      {#if !adminUnlocked}
        <div class="admin-login">
          <input class="admin-input" type="password" bind:value={adminPassword} placeholder="Enter admin password" on:keydown={e => e.key === 'Enter' && unlockAdmin()} />
      ...
      {:else}
        <div class="admin-panel">
          <div class="upload-area">
            <input type="file" id="file-upload" multiple accept="image/*" on:change={handleUpload} style="display:none" />
              ...
              <div class="admin-grid">
                {#each galleryImages as img}
                  <div class="admin-img-card">
                    ...
                    <input class="admin-label-input" type="text" placeholder="Add label (e.g. Teeth Whitening)" value={img.label} on:input={e => updateLabel(img.id, e.target.value)} />
...
<footer>
  <div class="footer-inner">
    ...
    <p>Caring for smiles in R.K. Puram, New Delhi. Your comfort and confidence is our priority.</p>
...
<style>
  ...
  :global(body) { font-family: 'DM Sans', sans-serif; background: #fdfcf9; color: #1a1a1a; overflow-x: hidden; }
  :global(a) { text-decoration: none; color: inherit; }
  :global(:root) {
    --teal: #0d9488; --teal-light: #f0fdf9; --teal-mid: #ccfbf1;
    --cream: #fdfcf9; --dark: #0f1a17; --text: #1a1a1a;
    --muted: #6b7280; --border: #e5e7eb; --card: #ffffff;
    --serif: 'Cormorant Garamond', Georgia, serif;
  }
  .nav { position: fixed; top: 0; left: 0; right: 0; z-index: 100; display: flex; align-items: center; gap: 32px; padding: 18px 5%; background: rgba(253,252,249,0.92); backdrop-filter: blur(12px); border-bottom: 1px solid var(--border); }
  .nav-logo { display: flex; align-items: center; gap: 10px; font-family: var(--serif); font-size: 1.05rem; font-weight: 600; }
  ...
  .nav-cta { display: inline-flex; align-items: center; gap: 6px; background: var(--teal); color: white; padding: 9px 18px; border-radius: 8px; font-size: 0.84rem; font-weight: 600; transition: all 0.2s; white-space: nowrap; }
  .nav-cta:hover { background: #0b7a6e; transform: translateY(-1px); }
  .hamburger { display: none; flex-direction: column; gap: 5px; background: none; border: none; cursor: pointer; padding: 4px; }
  .hamburger span { display: block; width: 22px; height: 2px; background: var(--text); border-radius: 2px; }
  .mobile-menu { position: fixed; top: 65px; left: 0; right: 0; z-index: 99; background: var(--cream); border-bottom: 1px solid var(--border); display: flex; flex-direction: column; padding: 16px 5%; }
  ...
  .btn-primary { display: inline-flex; align-items: center; gap: 8px; background: var(--teal); color: white; padding: 13px 26px; border-radius: 10px; font-size: 0.92rem; font-weight: 600; transition: all 0.2s; cursor: pointer; border: none; }
  .btn-primary:hover { background: #0b7a6e; transform: translateY(-2px); box-shadow: 0 8px 24px rgba(13,148,136,0.25); }
  .btn-ghost { display: inline-flex; align-items: center; gap: 8px; background: transparent; color: var(--text); border: 1.5px solid var(--border); padding: 13px 26px; border-radius: 10px; font-size: 0.92rem; font-weight: 500; transition: all 0.2s; }
  .btn-ghost:hover { border-color: var(--teal); color: var(--teal); }
  .hero { min-height: 100vh; display: grid; grid-template-columns: 1fr 420px; gap: 60px; align-items: center; padding: 120px 5% 80px; position: relative; overflow: hidden; background: linear-gradient(135deg, #f0fdf9 0%, #fdfcf9 60%); }
  .hero-bg { position: absolute; inset: 0; pointer-events: none; }
  .hero-circle { position: absolute; border-radius: 50%; background: radial-gradient(circle, rgba(13,148,136,0.08) 0%, transparent 70%); }
  .c1 { width: 700px; height: 700px; top: -200px; right: -100px; }
  .c2 { width: 400px; height: 400px; bottom: 0; left: -100px; }
  .hero-dots { position: absolute; inset: 0; background-image: radial-gradient(rgba(13,148,136,0.08) 1px, transparent 1px); background-size: 32px 32px; mask-image: radial-gradient(ellipse 80% 80% at 70% 50%, transparent 30%, black 100%); }
  .hero-content { position: relative; z-index: 1; }
  .hero-badge { display: inline-block; background: var(--teal-mid); color: var(--teal); padding: 6px 16px; border-radius: 100px; font-size: 0.78rem; font-weight: 600; letter-spacing: 0.05em; margin-bottom: 24px; }
  .hero h1 { font-family: var(--serif); font-size: clamp(2.8rem, 5vw, 4.2rem); font-weight: 700; line-height: 1.1; margin-bottom: 20px; color: var(--dark); }
  ...
  .hero-card { background: white; border: 1px solid var(--border); border-radius: 20px; padding: 28px; box-shadow: 0 20px 60px rgba(0,0,0,0.08); }
  .hcard-top { display: flex; gap: 14px; align-items: center; }
  .hcard-avatar { font-size: 2rem; }
  .hcard-name { font-family: var(--serif); font-size: 1rem; font-weight: 600; }
  .hcard-qual { font-size: 0.76rem; color: var(--muted); margin-top: 2px; }
  .hcard-divider { height: 1px; background: var(--border); margin: 16px 0; }
  ...
  .hcard-btn { display: block; text-align: center; margin-top: 18px; background: var(--teal-light); color: var(--teal); border: 1.5px solid var(--teal-mid); padding: 11px; border-radius: 10px; font-size: 0.88rem; font-weight: 600; transition: all 0.2s; }
  .hcard-btn:hover { background: var(--teal); color: white; }
  .section { padding: 100px 5%; max-width: 1200px; margin: 0 auto; }
  .section-header { text-align: center; margin-bottom: 64px; }
  .section-tag { display: inline-block; color: var(--teal); font-size: 0.78rem; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; margin-bottom: 12px; }
  .section-header h2 { font-family: var(--serif); font-size: clamp(2rem, 3.5vw, 2.8rem); font-weight: 700; color: var(--dark); margin-bottom: 14px; line-height: 1.2; }
  ...
  .services-section { background: #f8fffe; max-width: 100%; padding: 100px 5%; }
  .services-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; max-width: 1200px; margin: 0 auto; }
  .service-card { background: white; border: 1px solid var(--border); border-radius: 16px; padding: 28px; transition: all 0.25s; }
  ...
  .gallery-label { position: absolute; bottom: 0; left: 0; right: 0; background: linear-gradient(transparent, rgba(0,0,0,0.65)); color: white; padding: 20px 14px 12px; font-size: 0.84rem; font-weight: 500; }
  .gallery-empty { text-align: center; padding: 80px 20px; background: var(--teal-light); border-radius: 16px; border: 2px dashed var(--teal-mid); margin-bottom: 48px; }
  ...
  .about-card { background: linear-gradient(135deg, var(--teal) 0%, #0b7a6e 100%); border-radius: 24px; padding: 40px; color: white; text-align: center; }
  .about-avatar { font-size: 4rem; margin-bottom: 16px; }
  .about-badge { font-size: 0.8rem; opacity: 0.85; margin-bottom: 28px; font-style: italic; }
  ...
  .testi-card { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 28px; }
  ...
  .contact-card { background: white; border: 1px solid var(--border); border-radius: 16px; padding: 32px; text-align: center; }
  ...
  .ci-item { display: flex; gap: 14px; align-items: flex-start; font-size: 0.9rem; line-height: 1.7; }
  ...
  .admin-toggle-btn { background: none; border: 1px solid var(--border); color: var(--muted); padding: 8px 20px; border-radius: 8px; font-size: 0.82rem; cursor: pointer; transition: all 0.2s; }
  ...
  .upload-label { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8px; padding: 40px; border: 2px dashed var(--teal-mid); border-radius: 14px; background: white; cursor: pointer; transition: all 0.2s; text-align: center; }
  ...
  .upload-spinner { width: 28px; height: 28px; border-radius: 50%; border: 3px solid var(--teal-mid); border-top-color: var(--teal); animation: spin 0.8s linear infinite; }
  ...
  .wa-float { position: fixed; bottom: 28px; right: 28px; z-index: 999; width: 58px; height: 58px; border-radius: 50%; background: #25D366; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 20px rgba(37,211,102,0.4); transition: all 0.2s; }
  .wa-float:hover { transform: scale(1.1); box-shadow: 0 8px 30px rgba(37,211,102,0.5); }
  @media (max-width: 1024px) { .hero { grid-template-columns: 1fr; padding-top: 100px; } .hero-visual { display: none; } .about-section { grid-template-columns: 1fr; } .about-visual { display: none; } .services-grid { grid-template-columns: repeat(2, 1fr); } .testi-grid { grid-template-columns: 1fr; } }
  @media (max-width: 640px) { .nav-links, .nav-cta { display: none; } .hamburger { display: flex; } .services-grid { grid-template-columns: 1fr; } .contact-grid { grid-template-columns: 1fr; } .hero h1 { font-size: 2.4rem; } .hero-stats { flex-wrap: wrap; gap: 16px; } .footer-bottom { flex-direction: column; gap: 6px; align-items: center; } }
</style>
